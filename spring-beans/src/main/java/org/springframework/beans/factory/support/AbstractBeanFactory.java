...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		//Spring中的Bean可能存在别名,transformedBeanName方法为了得到真正的名称
		final String beanName = transformedBeanName(name);
		Object bean;


		//尝试从一级缓存池(单例池)中获取对象,当在初始化过程中无法获取到bean
		...
		if (sharedInstance != null && args == null) {
			...
			/**
			 * 尝试从AbstractBeanFactory的父容器中获取bean，
			 * 在只存在Spring时不存在父子容器，当Spring与SpringMvc整合以后存在父子容器概念
			 */
			...
			try {
				...
				//检查bean定义即检查这个Bean是不是抽象bean
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.
				//这里会检查某个bean的dependsOn属性，如果存在依赖，会先实例化依赖的bean,例如Person对象依赖Car对象,则会先创建car对象
