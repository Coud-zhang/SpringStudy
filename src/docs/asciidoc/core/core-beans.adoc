...
* <<beans-annotation-com.zkq.config,Annotation-based configuration>>: Spring 2.5 introduced
...
Typically, you define com.zkq.service layer objects, data access objects (DAOs), presentation
...
The following example shows the com.zkq.service layer objects `(services.xml)` configuration file:
...
In the preceding example, the com.zkq.service layer consists of the `PetStoreServiceImpl` class
...
example, `file:C:/com.zkq.config/services.xml` or `classpath:/com.zkq.config/services.xml`. However, be
...
----
	...
	PetStoreService com.zkq.service = context.getBean("petStore", PetStoreService.class);

	// use configured instance
	List<String> userList = com.zkq.service.getUsernameList();
----
====
...
<<core-convert-ConversionService-API, conversion com.zkq.service>> is used to convert these
...
----
	<bean id="mappings"
		class="org.springframework.beans.factory.com.zkq.config.PropertyPlaceholderConfigurer">
...
* Implement the more fine-grained control available with annotation-based configuration,
  as described in <<beans-annotation-com.zkq.config>>.
...
`org.springframework.beans.factory.com.zkq.config` package) to be useful.
====
...
`org.springframework.beans.factory.com.zkq.config.Scope` interface, which is described in this
section. For an idea of how to implement your own scopes, see the `Scope`
implementations that are supplied with the Spring Framework itself and the
{api-spring-framework}/beans/factory/com.zkq.config/Scope.html[`Scope`] javadoc,
...
See the {api-spring-framework}/beans/factory/com.zkq.config/Scope.html#registerDestructionCallback[javadoc]
...
{api-spring-framework}/beans/factory/com.zkq.config/BeanPostProcessor.html[`BeanPostProcessor`]
...
The `org.springframework.beans.factory.com.zkq.config.BeanPostProcessor` interface consists of
...
class itself or at least the `org.springframework.beans.factory.com.zkq.config.BeanPostProcessor`
...
----
	...
	import org.springframework.beans.factory.com.zkq.config.BeanPostProcessor;
...
`org.springframework.beans.factory.com.zkq.config.BeanFactoryPostProcessor`. The semantics of
...
{api-spring-framework}/beans/factory/com.zkq.config/BeanFactoryPostProcessor.html[`BeanFactoryPostProcessor`] and {api-spring-framework}/core/Ordered.html[`Ordered`] interfaces for more details.
...
[subs="verbatim,quotes"]
----
	<bean class="org.springframework.beans.factory.com.zkq.config.PropertyPlaceholderConfigurer">
...
See the {api-spring-framework}/beans/factory/com.zkq.config/PropertyPlaceholderConfigurer.html[`PropertyPlaceholderConfigurer`] javadoc for more information.
...
[[beans-annotation-com.zkq.config]]
...
[NOTE]
====
`<context:annotation-com.zkq.config/>` only looks for annotations on beans in the same
application context in which it is defined. This means that, if you put
`<context:annotation-com.zkq.config/>` in a `WebApplicationContext` for a `DispatcherServlet`,
...
(<<beans-annotation-com.zkq.config>>) demonstrates how to provide a lot of the configuration
...
more specific use cases (in the persistence, com.zkq.service, and presentation
...
choosing between using `@Component` or `@Service` for your com.zkq.service layer, `@Service` is
...
`<context:annotation-com.zkq.config>`. There is usually no need to include the
`<context:annotation-com.zkq.config>` element when using `<context:component-scan>`.
...
`CommonAnnotationBeanPostProcessor` by including the annotation-com.zkq.config attribute
...
being a sort of bonus there. For example, com.zkq.service components may expose management views
...
----
	<web-app>
		...
		<servlet>
			...
			<!-- Again, com.zkq.config locations must consist of one or more comma- or space-delimited
...
TIP: If you consistently refer to your types by a declared com.zkq.service interface, your
...
----
	...
	public Service userService() {
		UserService com.zkq.service = new SimpleUserService();
		// a reference to the proxied userPreferences bean
		com.zkq.service.setUserPreferences(userPreferences());
		return com.zkq.service;
	}
----
====
...
[[beans-java-further-information-java-com.zkq.config]]
...
normally expect to have two instances (one for each com.zkq.service). That definitely would be
...
----
	...
	public class ServiceConfig {
		...
		@Bean
		public TransferService transferService() {
			// navigate 'through' the com.zkq.config class to the @Bean method!
...
----
	...
	@Configuration
	@Import({ServiceConfig.class, DefaultRepositoryConfig.class})  // import the concrete com.zkq.config!
	public class SystemTestConfig {
...
include it within `system-test-com.zkq.config.xml` as a `<bean/>` definition. Because
`<context:annotation-com.zkq.config/>` is switched on, the container recognizes the
...
The following example shows part of a sample `system-test-com.zkq.config.xml` file:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.zkq.config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
NOTE: In `system-test-com.zkq.config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
describe in the previous example, we can redefine `system-test-com.zkq.config.xml` to take advantage of component-scanning.
Note that, in this case, we need not explicitly declare
`<context:annotation-com.zkq.config/>`, because `<context:component-scan/>` enables the same
functionality.

The following example shows the modified `system-test-com.zkq.config.xml` file:
...
----
	@Configuration
	@ImportResource("classpath:/com/acme/properties-com.zkq.config.xml")
	public class AppConfig {
...
user with a convenient com.zkq.service interface for configuring property sources and resolving
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			<jdbc:script location="classpath:com/bank/com.zkq.config/sql/schema.sql"/>
			<jdbc:script location="classpath:com/bank/com.zkq.config/sql/test-data.sql"/>
...
is populated with additional default property sources including servlet com.zkq.config and servlet
...
----
	<beans>
		<import resource="com/bank/com.zkq.service/${customer}-com.zkq.config.xml"/>
	</beans>
----
====
