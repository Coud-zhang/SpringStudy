...
* Provide declarative enterprise services. The most important such com.zkq.service is
...
multiple objects (such as all business operations in the com.zkq.service layer).
...
----
	...
	public class SystemArchitecture {
		...
		/**
		 * A join point is in the com.zkq.service layer if the method is defined
		 * in a type in the com.xyz.someapp.com.zkq.service package or any sub-package
		 * under that.
		 */
		@Pointcut("within(com.xyz.someapp.com.zkq.service..*)")
		...
		/**
		 * A business com.zkq.service is the execution of any method defined on a com.zkq.service
		 * interface. This definition assumes that interfaces are placed in the
		 * "com.zkq.service" package, and that implementation types are in sub-packages.
		 *
		 * If you group com.zkq.service interfaces by functional area (for example,
		 * in packages com.xyz.someapp.abc.com.zkq.service and com.xyz.someapp.def.com.zkq.service) then
		 * the pointcut expression "execution(* com.xyz.someapp..com.zkq.service.*.*(..))"
		 * could be used instead.
		 *
		 * Alternatively, you can write the expression using the 'bean'
		 * PCD, like so "bean(*Service)". (This assumes that you have
		 * named your Spring com.zkq.service beans in a consistent fashion.)
		 */
		@Pointcut("execution(* com.xyz.someapp..com.zkq.service.*.*(..))")
...
pointcut expression. For example, to make the com.zkq.service layer transactional, you could
write the following:
...
The `<aop:com.zkq.config>` and `<aop:advisor>` elements are discussed in <<aop-schema>>. The
...
[subs="verbatim"]
----
	execution(* com.xyz.com.zkq.service.AccountService.*(..))
----
====

* The execution of any method defined in the `com.zkq.service` package:
+
====
[source,java,indent=0]
[subs="verbatim"]
----
	execution(* com.xyz.com.zkq.service.*.*(..))
----
====

* The execution of any method defined in the com.zkq.service package or one of its sub-packages:
+
====
[source,java,indent=0]
[subs="verbatim"]
...
* Any join point (method execution only in Spring AOP) within the com.zkq.service package:
+
====
[source,java,indent=0]
[subs="verbatim,quotes"]
----
	within(com.xyz.com.zkq.service.*)
----
====

* Any join point (method execution only in Spring AOP) within the com.zkq.service package or one of its
sub-packages:
+
====
[source,java,indent=0]
...
[subs="verbatim,quotes"]
----
	this(com.xyz.com.zkq.service.AccountService)
----
====
+
...
[subs="verbatim,quotes"]
----
	target(com.xyz.com.zkq.service.AccountService)
----
====
+
...
the following aspect declares that all implementors of com.zkq.service interfaces also implement
...
----
	...
	public class UsageTracking {
		...
		@DeclareParents(value="com.xzy.myapp.com.zkq.service.*+", defaultImpl=DefaultUsageTracked.class)
...
before advice of the preceding example, com.zkq.service beans can be directly used as
...
each unique com.zkq.service object that executes a business com.zkq.service (each unique object bound to
'this' at join points matched by the pointcut expression). The aspect instance is
created the first time that a method is invoked on the com.zkq.service object. The aspect goes
out of scope when the com.zkq.service object goes out of scope. Before the aspect instance is
...
when the com.zkq.service object is the one with which this aspect is associated. See the AspectJ
...
multiple services in the com.zkq.service layer and, hence, is ideal for implementing through an
aspect.
...
----
	...
	<bean id="concurrentOperationExecutor" class="com.xyz.myapp.com.zkq.service.impl.ConcurrentOperationExecutor">
...
We can then use the annotation to annotate the implementation of com.zkq.service operations. The change
...
----
	@Around("com.xyz.myapp.SystemArchitecture.businessService() && " +
			"@annotation(com.xyz.myapp.com.zkq.service.Idempotent)")
	public Object doConcurrentOperation(ProceedingJoinPoint pjp) throws Throwable {
		...
	}
----
...
an `<aop:com.zkq.config>` element (you can have more than one `<aop:com.zkq.config>` element in an
application context configuration). An `<aop:com.zkq.config>` element can contain pointcut,
advisor, and aspect elements (note that these must be declared in that order).

WARNING: The `<aop:com.zkq.config>` style of configuration makes heavy use of Spring's
...
use either only the `<aop:com.zkq.config>` style or only the `AutoProxyCreator` style and
never mix them.
...
You can declare a named pointcut inside an `<aop:com.zkq.config>` element, letting the pointcut
definition be shared across several aspects and advisors.

A pointcut that represents the execution of any business com.zkq.service in the com.zkq.service layer can
...
----
	<aop:com.zkq.config>
		...
		<aop:pointcut id="businessService"
			expression="execution(* com.xyz.myapp.com.zkq.service.*.*(..))"/>
...
----
	<aop:com.zkq.config>
		...
		<aop:aspect id="myAspect" ref="aBean">
			...
			<aop:pointcut id="businessService"
				expression="execution(* com.xyz.myapp.com.zkq.service.*.*(..)) &amp;&amp; this(com.zkq.service)"/>
...
[subs="verbatim,quotes"]
----
	public void monitor(Object com.zkq.service) {
		...
	}
----
====
...
----
	<aop:com.zkq.config>
		...
		<aop:aspect id="myAspect" ref="aBean">
			...
			<aop:pointcut id="businessService"
				expression="execution(* com.xyz.myapp.com.zkq.service.*.*(..)) and this(com.zkq.service)"/>
...
Here, `dataAccessOperation` is the `id` of a pointcut defined at the top (`<aop:com.zkq.config>`)
...
----
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean id="personService" class="x.y.com.zkq.service.DefaultPersonService"/>
		...
		<aop:com.zkq.config>
			<aop:aspect ref="profiler">
				...
				<aop:pointcut id="theExecutionOfSomePersonServiceMethod"
					expression="execution(* x.y.com.zkq.service.PersonService.getPerson(String,int))
					and args(name, age)"/>
...
----
	...
	import x.y.com.zkq.service.PersonService;
...
`DefaultUsageTracked`, the following aspect declares that all implementors of com.zkq.service
...
----
	<aop:aspect id="usageTrackerAspect" ref="usageTracking">
		...
		<aop:declare-parents
			types-matching="com.xzy.myapp.com.zkq.service.*+"
			implement-interface="com.xyz.myapp.com.zkq.service.tracking.UsageTracked"
			default-impl="com.xyz.myapp.com.zkq.service.tracking.DefaultUsageTracked"/>
...
advice of the preceding example, com.zkq.service beans can be directly used as implementations of
...
----
	...
	<bean id="concurrentOperationExecutor"
		class="com.xyz.myapp.com.zkq.service.impl.ConcurrentOperationExecutor">
			<property name="maxRetries" value="3"/>
			<property name="order" value="100"/>
	</bean>
----
...
to annotate the implementation of com.zkq.service operations, as the following example shows:
...
----
	<aop:pointcut id="idempotentOperation"
			expression="execution(* com.xyz.myapp.com.zkq.service.*.*(..)) and
			@annotation(com.xyz.myapp.com.zkq.service.Idempotent)"/>
----
====
...
of the `<aop:com.zkq.config>` element to true, as follows:

====
[source,xml,indent=0]
[subs="verbatim,quotes"]
----
	<aop:com.zkq.config proxy-target-class="true">
...
[NOTE]
====
Multiple `<aop:com.zkq.config/>` sections are collapsed into a single unified auto-proxy creator
at runtime, which applies the _strongest_ proxy settings that any of the
`<aop:com.zkq.config/>` sections (typically from different XML bean definition files) specified.
...
`<aop:aspectj-autoproxy/>`, or `<aop:com.zkq.config/>` elements forces the use of CGLIB
...
In addition to declaring aspects in your configuration by using either `<aop:com.zkq.config>`
...
at the field or method level (see <<beans-annotation-com.zkq.config>> for further details).
...
----
	<bean id="myService"
			class="com.xzy.myapp.com.zkq.service.MyService"
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<!-- a com.zkq.service object; we will be profiling its methods -->
