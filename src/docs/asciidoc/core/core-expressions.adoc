...
By default, SpEL uses the conversion com.zkq.service available in Spring core
(`org.springframework.core.convert.ConversionService`). This conversion com.zkq.service comes
...
----
	...
	// "false" is passed in here as a String. SpEL and the conversion com.zkq.service
...
----
	...
	SpelParserConfiguration com.zkq.config = new SpelParserConfiguration(true,true);

	ExpressionParser parser = new SpelExpressionParser(com.zkq.config);
...
[subs="verbatim,quotes"]
----
	SpelParserConfiguration com.zkq.config = new SpelParserConfiguration(SpelCompilerMode.IMMEDIATE,
		this.getClass().getClassLoader());

	SpelExpressionParser parser = new SpelExpressionParser(com.zkq.config);
...
* Expressions relying on the conversion com.zkq.service
