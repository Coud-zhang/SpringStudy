...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.zkq.config.annotation.EnableWebSocket;
	import org.springframework.web.socket.com.zkq.config.annotation.WebSocketConfigurer;
	import org.springframework.web.socket.com.zkq.config.annotation.WebSocketHandlerRegistry;
...
[.small]#<<web-reactive.adoc#webflux-websocket-server-com.zkq.config, Same as in Spring WebFlux>>#
...
WebSocket Java com.zkq.config, as the following example shows:
...
that into Spring's `DefaultHandshakeHandler` through your WebSocket Java com.zkq.config.
...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.zkq.config.annotation.EnableWebSocketMessageBroker;
	import org.springframework.web.socket.com.zkq.config.annotation.StompEndpointRegistry;
	...
	public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
		...
		@Override
		public void configureMessageBroker(MessageBrokerRegistry com.zkq.config) {
			com.zkq.config.setApplicationDestinationPrefixes("/app"); // <2>
			com.zkq.config.enableSimpleBroker("/topic", "/queue"); // <3>
		}
	}
----
...
[[websocket-stomp-server-com.zkq.config]]
=== WebSocket Server
...
re-established. For example, a Stock Quote com.zkq.service that broadcasts stock quotes can
...
cloud-based STOMP com.zkq.service).
...
you need to ensure that the authentication `ChannelInterceptor` com.zkq.config is ordered
