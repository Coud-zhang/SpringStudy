...
This section starts by exposing the com.zkq.service to a remote client by using RMI and talk a bit
...
plain RMI in case of a traditional RMI com.zkq.service. The `RmiServiceExporter` explicitly
supports the exposing of any non-RMI services via RMI invokers.

We first have to set up our com.zkq.service in the Spring container.
...
Next, we have to expose our com.zkq.service by using `RmiServiceExporter`.
The following example shows how to do so:
...
----
	<bean class="org.springframework.remoting.rmi.RmiServiceExporter">
		...
		<property name="com.zkq.service" ref="accountService"/>
...
Furthermore, the com.zkq.service name is used to bind the com.zkq.service. So, in the preceding example, the
com.zkq.service is bound at `'rmi://HOST:1199/AccountService'`. We use this URL later on to link in
the com.zkq.service at the client side.

NOTE: The `servicePort` property has been omitted (it defaults to 0). This means that an
anonymous port is used to communicate with the com.zkq.service.
...
To link in the com.zkq.service on the client, we create a separate Spring container,
to contain the following simple object and the com.zkq.service linking configuration bits:
...
That is all we need to do to support the remote account com.zkq.service on the client. Spring
transparently creates an invoker and remotely enables the account com.zkq.service through the
...
Now we are ready to link in the com.zkq.service at the client. No explicit handler mapping is
specified (to map request URLs onto services), so we use `BeanNameUrlHandlerMapping`
used. Hence, the com.zkq.service is exported at the URL indicated through its bean name
...
By using the `HessianProxyFactoryBean`, we can link in the com.zkq.service at the client. The same
...
Setting up the HTTP invoker infrastructure for a com.zkq.service object closely resembles the
...
Again, linking in the com.zkq.service from the client much resembles the way you would do it
when you use Hessian. By using a proxy, Spring can translate your calls to
HTTP POST requests to the URL that points to the exported com.zkq.service. The following example
...
----
	 ...
	 * endpoint classes. If an existing com.zkq.service needs to be exported, a wrapper that
	 ...
	 * accordingly. The servlet name usually needs to match the specified WS com.zkq.service name.
	 *
	 * The web com.zkq.service engine manages the lifecycle of instances of this class.
...
See the various Java EE web com.zkq.service tutorials for details.
...
shown earlier), you have one servlet definition per com.zkq.service endpoint, with each endpoint
...
Spring provides two factory beans to create JAX-WS web com.zkq.service proxies, namely
`LocalJaxWsServiceFactoryBean` and `JaxWsPortProxyFactoryBean`. The former can
return only a JAX-WS com.zkq.service class for us to work with. The latter is the full-fledged
version that can return a proxy that implements our business com.zkq.service interface.
...
.wsdl file. `serviceName` corresponds to the com.zkq.service name in the .wsdl file. `portName`
corresponds to the port name in the .wsdl file.

Accessing the web com.zkq.service is easy, as we have a bean factory for it that exposes it as
...
----
	<bean id="client" class="example.AccountClientImpl">
		...
		<property name="com.zkq.service" ref="accountWebService"/>
	</bean>
----
====

From the client code, we can access the web com.zkq.service as if it were a normal class,
as the following example shows:
...
----
	public class AccountClientImpl {
		...
		private AccountService com.zkq.service;

		public void setService(AccountService com.zkq.service) {
			this.com.zkq.service = com.zkq.service;
		}

		public void foo() {
			com.zkq.service.insertAccount(...);
		}
	}
----
====
...
On the server, you need to expose the com.zkq.service object that uses the
...
----
	...
	public class Client {
		...
		public static void main(String[] args) throws Exception {
			...
			CheckingAccountService com.zkq.service = (CheckingAccountService) ctx.getBean("checkingAccountService");
			com.zkq.service.cancelAccount(new Long(10));
		}
	}
----
====
...
in the local case. However, when you export a remote com.zkq.service, you should expose a specific
com.zkq.service interface, with specific operations intended for remote usage. Besides internal
...
com.zkq.service interface to be specified.

This is a trade-off between configuration convenience and the risk of accidental
exposure of internal methods. Always specifying a com.zkq.service interface is not too much
...
<<web.adoc#mvc-com.zkq.config-message-converters, Configuring Message Converters>>).
...
switch to a POJO (plain old Java object) implementation of the com.zkq.service if it makes sense
...
is a subclass of `RuntimeException`. You can then switch the target com.zkq.service at will
...
for managing JMS destinations, such as providing a com.zkq.service locator for destinations
stored in JNDI.
...
The `org.springframework.jms.com.zkq.config` package provides the parser implementation for the
`jms` namespace as well as the java com.zkq.config support to configure listener containers and
...
`JndiDestinationResolver` is also provided to act as a com.zkq.service locator for
...
----
	...
	<bean id="jmsListenerContainerFactory"
			class="org.springframework.jms.com.zkq.config.DefaultJmsListenerContainerFactory">
		<property name="connectionFactory" ref="connectionFactory"/>
...
case (through setting the 'proxy-target-class' flag on `<aop:com.zkq.config/>`,
...
`com.zkq.service:jmx:jmxmp://localhost:9875`. The `serverConnector` bean thus exposes the
...
----
	<bean id="serverConnector"
		...
		<property name="serviceUrl"
				value="com.zkq.service:jmx:rmi://localhost/jndi/rmi://localhost:1099/myconnector"/>
	</bean>
----
====
...
----
	<bean id="serverConnector"
		...
		<property name="serviceUrl"
			value="com.zkq.service:jmx:iiop://localhost/jndi/iiop://localhost:900/myconnector"/>
...
Note that, when you use a RMI-based connector, you need the lookup com.zkq.service (`tnameserv` or
...
----
	<bean id="clientConnector" class="org.springframework.jmx.support.MBeanServerConnectionFactoryBean">
		<property name="serviceUrl" value="com.zkq.service:jmx:rmi://localhost/jndi/rmi://localhost:1099/jmxrmi"/>
	</bean>
----
====
...
----
	<bean id="serverConnector" class="org.springframework.jmx.support.ConnectorServerFactoryBean">
		...
		<property name="serviceUrl" value="com.zkq.service:jmx:burlap://localhost:9874"/>
	</bean>
----
====
...
----
	<bean id="clientConnector"
		...
		<property name="serviceUrl" value="com.zkq.service:jmx:rmi://remotehost:9875"/>
	</bean>
...
* <<cci-com.zkq.config-connector>>
* <<cci-com.zkq.config-connectionfactory>>
* <<cci-com.zkq.config-cci-connections>>
* <<cci-com.zkq.config-single-connection>>
...
* `WorkManagerTaskExecutor`:
  This implementation uses a CommonJ `WorkManager` as its backing com.zkq.service provider
...
As with other services in the Spring Framework, the caching com.zkq.service is an abstraction
...
[[cache-annotations-com.zkq.config]]
...
[subs="verbatim"]
----
	<!-- the com.zkq.service we want to make cacheable -->
	<bean id="bookService" class="x.y.com.zkq.service.DefaultBookService"/>
...
The `aop:com.zkq.config` definition applies the cache advice to the appropriate points in the
...
----
	...
	<bean id="ehcache"
			class="org.springframework.cache.ehcache.EhCacheManagerFactoryBean" p:com.zkq.config-location="ehcache.xml"/>
----
====
