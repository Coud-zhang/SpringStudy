...
set up com.zkq.service locator registries and similar structures).
...
you can test com.zkq.service layer objects by stubbing or mocking DAO or repository interfaces,
...
----
	@ContextConfiguration("/test-com.zkq.config.xml") <1>
...
----
	@ContextHierarchy({
		@ContextConfiguration("/parent-com.zkq.config.xml"),
		@ContextConfiguration("/child-com.zkq.config.xml")
	})
...
----
	...
	@Sql(
		...
		com.zkq.config = @SqlConfig(commentPrefix = "`", separator = "@@") <1>
	)
	public void userTest {
...
----
	@Test
	@SqlGroup({ <1>
		@Sql(scripts = "/test-schema.sql", com.zkq.config = @SqlConfig(commentPrefix = "`")),
...
[subs="verbatim,quotes"]
----
	@SpringJUnitConfig(locations = "/test-com.zkq.config.xml") <1>
...
[subs="verbatim,quotes"]
----
	@SpringJUnitWebConfig(locations = "/test-com.zkq.config.xml") <1>
...
----
	...
	@ContextConfiguration({"/app-com.zkq.config.xml", "/test-data-access-com.zkq.config.xml"})
	@ActiveProfiles("dev")
	@Transactional
...
the `TestContextManager` with which the listener is registered. See <<testcontext-tel-com.zkq.config>>.
...
[[testcontext-tel-com.zkq.config]]
...
[[testcontext-tel-com.zkq.config-registering-tels]]
...
[[testcontext-tel-com.zkq.config-automatic-discovery]]
...
[[testcontext-tel-com.zkq.config-ordering]]
...
through the <<testcontext-tel-com.zkq.config-automatic-discovery, aforementioned>>
...
[[testcontext-tel-com.zkq.config-merging]]
...
the aforementioned <<testcontext-tel-com.zkq.config-automatic-discovery, automatic discovery
mechanism>>.
...
of `AnnotationAwareOrderComparator`, as described in <<testcontext-tel-com.zkq.config-ordering>>.
...
* <<testcontext-ctx-management-mixed-com.zkq.config>>
...
is treated as an absolute classpath location (for example, `/org/example/com.zkq.config.xml`). A
...
----
	@RunWith(SpringRunner.class)
	// ApplicationContext will be loaded from "/app-com.zkq.config.xml" and
	// "/test-com.zkq.config.xml" in the root of the classpath
	@ContextConfiguration(locations={"/app-com.zkq.config.xml", "/test-com.zkq.config.xml"}) <1>
	public class MyTest {
		// class body...
	}
----
...
----
	@RunWith(SpringRunner.class)
	@ContextConfiguration({"/app-com.zkq.config.xml", "/test-com.zkq.config.xml"}) <1>
	public class MyTest {
		// class body...
	}
----
...
----
	@RunWith(SpringRunner.class)
	// ApplicationContext will be loaded from
	// "/app-com.zkq.config.xml" and "/TestConfig.groovy"
	@ContextConfiguration({ "/app-com.zkq.config.xml", "/TestConfig.groovy" })
	public class MyTest {
		// class body...
	}
----
...
`ExtendedTest` is loaded from `base-com.zkq.config.xml` and `extended-com.zkq.config.xml`, in that order.
Beans defined in `extended-com.zkq.config.xml` can, therefore, override (that is, replace) those
defined in `base-com.zkq.config.xml`. The following example shows how one class can extend
...
----
	@RunWith(SpringRunner.class)
	// ApplicationContext will be loaded from "/base-com.zkq.config.xml"
	// in the root of the classpath
	@ContextConfiguration("/base-com.zkq.config.xml") <1>
	public class BaseTest {
		// class body...
	}

	// ApplicationContext will be loaded from "/base-com.zkq.config.xml" and
	// "/extended-com.zkq.config.xml" in the root of the classpath
	@ContextConfiguration("/extended-com.zkq.config.xml") <2>
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean id="transferService"
				class="com.bank.com.zkq.service.internal.DefaultTransferService">
		...
		<bean id="feePolicy"
			class="com.bank.com.zkq.service.internal.ZeroFeePolicy"/>

		<beans profile="dev">
			<jdbc:embedded-database id="dataSource">
				<jdbc:script
					location="classpath:com/bank/com.zkq.config/sql/schema.sql"/>
				<jdbc:script
					location="classpath:com/bank/com.zkq.config/sql/test-data.sql"/>
...
[subs="verbatim,quotes"]
----
	package com.bank.com.zkq.service;

	@RunWith(SpringRunner.class)
	// ApplicationContext will be loaded from "classpath:/app-com.zkq.config.xml"
	@ContextConfiguration("/app-com.zkq.config.xml")
	@ActiveProfiles("dev")
...
`app-com.zkq.config.xml` configuration file in the root of the classpath. If you inspect
`app-com.zkq.config.xml`, you can see that the `accountRepository` bean has a dependency on a
	...
	public class StandaloneDataConfig {
		...
		public DataSource dataSource() {
			return new EmbeddedDatabaseBuilder()
				...
				.addScript("classpath:com/bank/com.zkq.config/sql/schema.sql")
				.addScript("classpath:com/bank/com.zkq.config/sql/test-data.sql")
				.build();
		}
	}
----
...
[subs="verbatim,quotes"]
----
	package com.bank.com.zkq.service.test;
...
----
	...
	// classpath resource
	@ContextConfiguration("/spring/test-servlet-com.zkq.config.xml")
...
----
	...
	// file system resource
	@ContextConfiguration("file:src/main/webapp/WEB-INF/servlet-com.zkq.config.xml")
...
For example, if `TestClassA` specifies `{"app-com.zkq.config.xml", "test-com.zkq.config.xml"}` for the
...
`{"app-com.zkq.config.xml", "test-com.zkq.config.xml"}` for its locations (either explicitly or
...
are loaded: one for `/app-com.zkq.config.xml`, one for `/user-com.zkq.config.xml`, and one for
`{"/user-com.zkq.config.xml", "/order-com.zkq.config.xml"}`. As with the previous example, the
application context loaded from `/app-com.zkq.config.xml` is set as the parent context for the
contexts loaded from `/user-com.zkq.config.xml` and `{"/user-com.zkq.config.xml", "/order-com.zkq.config.xml"}`.
...
application context for `ExtendedTests` is loaded only from `/test-user-com.zkq.config.xml` and
has its parent set to the context loaded from `/app-com.zkq.config.xml`. The following listing
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-user-com.zkq.config.xml",
...
----
	...
	@ContextConfiguration("repository-com.zkq.config.xml")
...
`@ContextConfiguration` annotation (that is, `repository-com.zkq.config.xml`). The following
...
`userService`, we are assured that the user com.zkq.service has access to the request-scoped
...
the user com.zkq.service has access to the session-scoped `userPreferences` for the current
...
[[testcontext-tx-mgr-com.zkq.config]]
...
----
	...
	@Sql(
		...
		com.zkq.config = @SqlConfig(transactionMode = ISOLATED)
	)
	@Sql(
...
declared directly by using the `com.zkq.config` attribute of the `@Sql` annotation, `@SqlConfig`
...
a mock com.zkq.service with Mockito:
...
You can then inject the mock com.zkq.service into the test to set up and verify your
