...
public class HttpComponentsHttpInvokerRequestExecutorTests {
	...
	public void customizeConnectionTimeout() throws IOException {
		...
		HttpInvokerClientConfiguration config = mockHttpInvokerClientConfiguration("http://fake-com.zkq.service");
	...
	public void mergeBasedOnCurrentHttpClient() throws Exception {
		...
		// Update the Http client so that it returns an updated  com.zkq.config
	...
	public void ignoreFactorySettings() throws IOException {
		...
		HttpInvokerClientConfiguration config = mockHttpInvokerClientConfiguration("http://fake-com.zkq.service");
		HttpPost httpPost = executor.createHttpPost(config);
		assertNull("custom request com.zkq.config should not be set", httpPost.getConfig());
	}
