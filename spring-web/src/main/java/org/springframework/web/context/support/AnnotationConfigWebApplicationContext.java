 ...
 * for registering classes one by one (specifying class names as com.zkq.config location) as well
 * as for classpath scanning (specifying base packages as com.zkq.config location).
 *
		...
		if (configLocations != null) {
			for (String configLocation : configLocations) {
				try {
					...
					if (logger.isTraceEnabled()) {
						logger.trace("Could not load class for com.zkq.config location [" + configLocation +
