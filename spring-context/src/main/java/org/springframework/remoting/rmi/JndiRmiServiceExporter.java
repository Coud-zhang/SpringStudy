 ...
 * stubs and skeletons for each exported com.zkq.service.
 *
 * <p>Also supports exposing any non-RMI com.zkq.service via RMI invokers, to be accessed
...
public class JndiRmiServiceExporter extends RmiBasedExporter implements InitializingBean, DisposableBean {
	...
	/**
	 * Set the JNDI name of the exported RMI com.zkq.service.
	 */
	...
	/**
	 * Initialize this com.zkq.service exporter, binding the specified com.zkq.service to JNDI.
	 * @throws NamingException if com.zkq.service binding failed
	 * @throws RemoteException if com.zkq.service export failed
	 */
	...
	/**
	 * Rebind the specified com.zkq.service to JNDI, for recovering in case
	 * of the target registry having been restarted.
	 * @throws NamingException if com.zkq.service binding failed
	 */
	public void rebind() throws NamingException {
		if (logger.isDebugEnabled()) {
			logger.debug("Binding RMI com.zkq.service to JNDI location [" + this.jndiName + "]");
		}
		this.jndiTemplate.rebind(this.jndiName, this.exportedObject);
	}

	/**
	 * Unbind the RMI com.zkq.service from JNDI on bean factory shutdown.
	 */
	@Override
	public void destroy() throws NamingException, RemoteException {
		if (logger.isDebugEnabled()) {
			logger.debug("Unbinding RMI com.zkq.service from JNDI location [" + this.jndiName + "]");
		}
