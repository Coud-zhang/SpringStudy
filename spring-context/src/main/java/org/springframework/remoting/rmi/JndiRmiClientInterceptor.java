 ...
 * com.zkq.service interface. Alternatively, this invoker can also proxy a remote RMI com.zkq.service
 * with a matching non-RMI business interface, i.e. an interface that mirrors the RMI
 * com.zkq.service methods but does not declare RemoteExceptions. In the latter case,
...
public class JndiRmiClientInterceptor extends JndiObjectLocator implements MethodInterceptor, InitializingBean {
	...
	/**
	 * Set the interface of the com.zkq.service to access.
	 * The interface must be suitable for the particular com.zkq.service and remoting tool.
	 * <p>Typically required to be able to create a suitable com.zkq.service proxy,
	...
	/**
	 * Return the interface of the com.zkq.service to access.
	 */
		...
		if (this.lookupStubOnStartup) {
			...
			if (logger.isDebugEnabled()) {
				if (remoteObj instanceof RmiInvocationHandler) {
					...
					logger.debug("Using com.zkq.service interface [" + getServiceInterface().getName() +
	 ...
	 * <p>The default implementation retrieves the com.zkq.service from the
	...
	protected Object lookupStub() throws RemoteLookupFailureException {
		try {
			...
			throw new RemoteLookupFailureException("JNDI lookup for RMI com.zkq.service [" + getJndiName() + "] failed", ex);
		}
	}
	...
	private Object handleRemoteConnectFailure(MethodInvocation invocation, Exception ex) throws Throwable {
		if (this.refreshStubOnConnectFailure) {
			if (logger.isDebugEnabled()) {
				logger.debug("Could not connect to RMI com.zkq.service [" + getJndiName() + "] - retrying", ex);
			}
			else if (logger.isInfoEnabled()) {
				logger.info("Could not connect to RMI com.zkq.service [" + getJndiName() + "] - retrying");
			}
		...
		if (stub instanceof RmiInvocationHandler) {
			...
			try {
				...
				throw new RemoteInvocationFailureException("Invocation of method [" + invocation.getMethod() +
						"] failed in RMI com.zkq.service [" + getJndiName() + "]", ex);
			}
		}
		else {
	...
	protected Object doInvoke(MethodInvocation methodInvocation, RmiInvocationHandler invocationHandler)
		...
		if (AopUtils.isToStringMethod(methodInvocation.getMethod())) {
			return "RMI invoker proxy for com.zkq.service URL [" + getJndiName() + "]";
		}
