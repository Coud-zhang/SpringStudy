 ...
 * and RMI invokers. Exposes the proxied com.zkq.service for use as a bean reference,
 * using the specified com.zkq.service interface. Proxies will throw Spring's unchecked
 * RemoteAccessException on remote invocation failure instead of RMI's RemoteException.
 *
 * <p>The com.zkq.service URL must be a valid RMI URL like "rmi://localhost:1099/myservice".
 * RMI invokers work at the RmiInvocationHandler level, using the same invoker stub
 * for any com.zkq.service. Service interfaces do not have to extend {@code java.rmi.Remote}
 ...
 * RMI com.zkq.service interface. Alternatively, this factory can also proxy a remote RMI
 * com.zkq.service with a matching non-RMI business interface, i.e. an interface that mirrors
 * the RMI com.zkq.service methods but does not declare RemoteExceptions. In the latter case,
