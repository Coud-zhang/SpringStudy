...
public class ImportTests {
	...
	public void testProcessImportsWithAsm() {
		...
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("com.zkq.config", new RootBeanDefinition(ConfigurationWithImportAnnotation.class.getName()));
		ConfigurationClassPostProcessor pp = new ConfigurationClassPostProcessor();
		pp.postProcessBeanFactory(beanFactory);
	...
	public void testImportAnnotationWithMultipleArgumentsResultingInOverriddenBeanDefinition() {
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("com.zkq.config", new RootBeanDefinition(
	...
	public void reproSpr9023() {
		...
		assertThat(ctx.getBeanNamesForType(B.class)[0], is("com.zkq.config-b"));
		assertThat(ctx.getBeanNamesForType(A.class)[0], is("com.zkq.config-a"));
	}

	@Configuration("com.zkq.config-a")
	static class A { }

	@Configuration("com.zkq.config-b")
	@Import(A.class)
	static class B { }
