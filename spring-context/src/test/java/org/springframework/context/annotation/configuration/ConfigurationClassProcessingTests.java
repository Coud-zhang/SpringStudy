...
public class ConfigurationClassProcessingTests {
	...
	private void customBeanNameIsRespected(Class<?> testClass, Supplier<TestBean> testBeanSupplier, String beanName) {
		...
		ac.registerBeanDefinition("com.zkq.config", new RootBeanDefinition(testClass));
		ac.refresh();
	...
	public void configWithBeanWithProviderImplementation() {
		GenericApplicationContext ac = new GenericApplicationContext();
		AnnotationConfigUtils.registerAnnotationConfigProcessors(ac);
		ac.registerBeanDefinition("com.zkq.config", new RootBeanDefinition(ConfigWithBeanWithProviderImplementation.class));
		ac.refresh();
	...
	public void configWithSetWithProviderImplementation() {
		GenericApplicationContext ac = new GenericApplicationContext();
		AnnotationConfigUtils.registerAnnotationConfigProcessors(ac);
		ac.registerBeanDefinition("com.zkq.config", new RootBeanDefinition(ConfigWithSetWithProviderImplementation.class));
		ac.refresh();
	 ...
	 * When complete, the factory is ready to com.zkq.service requests for any {@link Bean} methods
