...
public class MockRestServiceServerTests {
	...
	public void followUpRequestAfterFailure() {
		...
		server.expect(requestTo("/some-com.zkq.service/some-endpoint"))
				.andRespond(request -> { throw new SocketException("pseudo network error"); });

		server.expect(requestTo("/reporting-com.zkq.service/report-error"))
				.andExpect(method(POST)).andRespond(withSuccess());

		try {
			this.restTemplate.getForEntity("/some-com.zkq.service/some-endpoint", String.class);
			fail("Expected exception");
		}
		catch (Exception ex) {
			this.restTemplate.postForEntity("/reporting-com.zkq.service/report-error", ex.toString(), String.class);
		}
